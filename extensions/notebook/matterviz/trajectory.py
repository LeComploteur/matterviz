...
class TrajectoryWidget(anywidget.AnyWidget):
    ...
    Examples:
        Basic usage:
        >>> from matterviz import TrajectoryWidget
    ...
    _esm = (_STATIC_DIR / "trajectory.js").read_text()
    _css = (_STATIC_DIR / "trajectory.css").read_text()
    ...
    display_mode = t.Unicode("structure+scatter").tag(
        sync=True
    ...
    step_labels = t.Union([t.Int(), t.List()], allow_none=True, default_value=5).tag(
        sync=True
    )
    ...
    def _normalize_trajectory(self, trajectory: Any) -> dict[str, Any] | None:
        ...
        try:
            from matterviz.mime import is_ase_atoms
        ...
        try:
            from matterviz.mime import is_pymatgen_structure
    ...
    def _list_to_trajectory_dict(self, structures: list[Any]) -> dict[str, Any]:
        ...
        trajectory_dict = {"frames": [], "metadata": {}}
            ...
            if hasattr(atoms, "get_forces"):
                try:
                    ...
                    if forces is not None:
                        ...
                        metadata["force_norm"] = float(
                            (sum(fn**2 for fn in force_norms)) ** 0.5
                        )
                except Exception:
                    pass
    ...
    def update_trajectory(
        self, new_trajectory: dict[str, Any] | list[Any] | Any
    ) -> None:
    ...
    @property
    def current_structure(self) -> dict[str, Any] | None:
        """Get the structure for the current step."""
        if (
            self.trajectory
            and "frames" in self.trajectory
            and 0 <= self.current_step_idx < len(self.trajectory["frames"])
        ):
